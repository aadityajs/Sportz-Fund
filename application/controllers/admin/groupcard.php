...
class Groupcard extends CI_Controller {
...
public function __construct()
	   {
			...
			$this->load->model('admingroupcardmodel');
	...
	public function showmlb() {
		...
		$data['mlbgamedate'] = $this->admingroupcardmodel->listMlbGamedate();
		
		$this->load->view('backend/list-mlbgroupcard', $data);
		//exit;
	}
	...
	public function shownfl() {
		...
		$data['nflgamedate'] = $this->admingroupcardmodel->listNflGamedate();
		//print_r($data['allwinner']);
		//exit;
		$this->load->view('backend/list-nflgroupcard', $data);
		//exit;
	}
	...
	public function add() {
		...
		$data['editGamedate'] = '';
		$data['getscoringperiod'] = $this->admindonationmodel->listAllDonation1();
		//print_r($data['getscoringperiod']);exit;
		 $submit=$this->input->post('groupcardsubmit');
		if($submit) {
		...
		/*if(empty($player_name))
			...
			if(empty($type))
			{
				...
				redirect("admin/groupcard/add");
			}

			//make registration for user and store to db
			$this->admingroupcardmodel->add_gamedate();
			if($type=='MLB')
			{
		...
		$this->load->view('backend/add-groupcard', $data);
	}
	...
	public function delete($game_id,$type) {
		$$data['admindetail']=$this->adminmodel->showCurrentAdmin();
		$this->admingroupcardmodel->deleteGamedate($game_id,$type);
		//exit;
	}
