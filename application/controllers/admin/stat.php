...
public function __construct()
	   {
			...
			$this->load->model('adminstatmodel');
	...
	public function showmlb() {
		...
		$data['mlbgamedate'] = $this->adminstatmodel->listMlbGamedate();
		//print_r($data['allwinner']);
		//exit;
		$this->load->view('backend/list-mlbgamestat', $data);
		//exit;
	}
	...
	public function shownfl() {
		...
		$data['nflgamedate'] = $this->adminstatmodel->listNflGamedate();
		//print_r($data['allwinner']);
		//exit;
		$this->load->view('backend/list-nflgamestat', $data);
		//exit;
	}
	...
	//echo $sp_id;exit;
		...
		$data['editPlayer'] = $this->adminstatmodel->get_scoring_details($sp_id,$type);

		//print_r($data['editPlayer'][0]);

		$submit=$this->input->post('pointsubmit');

		if($submit) {
				$this->adminstatmodel->editScoring($sp_id,$type,$this->input->post());
	...
	public function status($stat,$id,$type) {
		$data['admindetail']=$this->adminmodel->showCurrentAdmin();
		$this->adminstatmodel->chngStatus($stat,$id,$type);
